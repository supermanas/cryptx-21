<?php
 ...
 if(check_ban($id,$db)){
  ...
  if(isset($_POST['but'])){
    ...
    make_log($level[1],$answer,40,$time);

    if($answer == "kevinmichaelcostner"){
      level_up($id,40,$conn);
      header("Location: ../play.php");
    }else{
      $rekt = 1;
...
<link rel="icon" type="image/png" href="http://createchclub.xyz/wp-content/uploads/2019/02/cog@2x.png">
    <title>De-CryptX | Level - 40</title>
...
<section>
    <div class="container" align="center" >
        ...
        <form name = "main" action = "index.php" method = "post" class="login-email">
            <p class="login-text" style="font-size: 2rem; font-weight: 800;">level-40</p>
          <p class="bottom-three"style="color:white;font-size: 2rem;font-weight: 800;">
   Jumped from a flying vehicle, onto a landmass isolated. Water surrounding a centurian warriors. its a war for the royalty, one only for 14 days.
</p>
